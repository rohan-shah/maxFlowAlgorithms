...
{
	template<class graphType> struct updateFlowIncrementalArgs
	{
		updateFlowIncrementalArgs(const graphType& graph, edmondsKarpMaxFlowScratch<graphType>& scratch)
		...
		double newCapacity;
		edmondsKarpMaxFlowScratch<graphType>& scratch;
		int source, sink;
		std::vector<double> working1, working2, working3;
		int nDirectedEdges;
	};
	template<class graphType> void updateFlowIncremental(updateFlowIncrementalArgs<graphType>& args, double previousMaxFlow, double& newMaxFlow)
	{
